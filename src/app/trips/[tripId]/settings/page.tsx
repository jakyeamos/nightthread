...
import { addCity, assignCityFromDay, createDaysFromTripDates, moveCity, updateCity, updateTripBasics } from "@/app/trips/[tripId]/actions";
...
export default async function SettingsPage({ params }: { params: Promise<{ tripId: string }> }) {
  ...
  const [trip, cityRows, dayCount, members, invitation] = await Promise.all([
    ...
    env.DB.prepare(`select c.id,c.name,c.country_code as countryCode,c.timezone as timeZone,c.lat,c.lon,min(d.ordinal) as firstDay,max(d.ordinal) as lastDay from cities c left join trip_days d on d.city_id=c.id and d.trip_id=c.trip_id where c.trip_id=?1 and c.deleted_at is null group by c.id order by c.position`).bind(tripId).all<CityRow>(),
