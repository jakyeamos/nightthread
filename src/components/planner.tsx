...
import { demoDays, demoIdeas, demoItems, type DemoIdea, type DemoItem } from "@/lib/demo-data";
...
function IdeaCard({ idea, onSchedule }: { idea: DemoIdea; onSchedule: (idea: DemoIdea) => void }) {
  ...
  return (
    <article ref={draggable.setNodeRef} style={{ transform: draggable.transform ? `translate3d(${draggable.transform.x}px, ${draggable.transform.y}px, 0)` : undefined }} className={`group overflow-hidden rounded-xl border border-[var(--line)] bg-[var(--night-raised)] ${draggable.isDragging ? "relative z-50 opacity-70" : ""}`}>
      <div className="relative h-28 overflow-hidden"><img src={idea.image} alt="" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-[1.03]" /><span className={`absolute left-2 top-2 rounded-md px-2 py-1 text-[10px] font-semibold ${idea.priority === "must_do" ? "bg-[var(--yarn)] text-white" : "bg-black/70 text-white"}`}>{priorityLabel[idea.priority]}</span>{idea.scheduled && <span className="absolute right-2 top-2 flex items-center gap-1 rounded-md bg-[oklch(0.18_0.05_160/.9)] px-2 py-1 text-[10px] text-[var(--positive)]"><Check size={11} />Scheduled</span>}<a href="https://unsplash.com" target="_blank" rel="noreferrer" className="absolute bottom-1.5 right-2 text-[8px] text-white/70 hover:text-white">Unsplash</a></div>
      <div className="p-3"><h3 className="truncate text-sm font-semibold">{idea.name}</h3><p className="muted mt-1 truncate text-xs">{idea.detail}</p><div className="mt-3 flex items-center justify-between"><VoteButton initialCount={idea.votes} /><div className="flex items-center"><button ref={draggable.setActivatorNodeRef} {...draggable.listeners} {...draggable.attributes} disabled={idea.scheduled} aria-label={`Drag ${idea.name} to a day`} className="grid size-8 place-items-center rounded-lg text-[var(--faint)] hover:bg-[var(--night-soft)] hover:text-white disabled:opacity-30"><GripVertical size={14} /></button><button type="button" disabled={idea.scheduled} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); onSchedule(idea); }} className="flex h-8 items-center gap-1 rounded-lg px-2 text-xs text-[var(--muted)] hover:bg-[var(--night-soft)] hover:text-white disabled:opacity-30"><CalendarPlus size={14} />Add</button></div></div></div>
    </article>
  );
}
...
function ItemCard({ item, onDelete }: { item: DemoItem; onDelete: (item: DemoItem) => void }) {
  ...
  return <motion.article layout initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: 30 }} className={`group relative grid grid-cols-[60px_1fr_auto] items-start gap-3 rounded-xl border p-3.5 ${placeholder ? "border-dashed border-[oklch(0.42_0.07_285)] bg-[oklch(0.16_0.035_285)]" : "border-[var(--line)] bg-[var(--night-raised)]"}`}>
    <div><p className="text-sm font-semibold tabular-nums">{item.start}</p><p className="muted mt-1 text-[10px]">{item.duration} min</p></div>
    <div className="min-w-0"><div className="flex items-center gap-2"><h3 className="truncate text-sm font-semibold">{item.title}</h3>{placeholder && <span className="rounded-md bg-[var(--indigo-soft)] px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-[oklch(0.79_0.1_285)]">Open</span>}</div><p className="muted mt-1 flex items-center gap-1 text-xs">{placeholder ? <Sparkles size={11} /> : <MapPin size={11} />}{item.subtitle}</p><div className="mt-2 flex flex-wrap gap-2">{item.reservation === "needed" && <span className="rounded-md bg-[var(--yarn-soft)] px-2 py-1 text-[10px] text-[oklch(0.82_0.13_18)]">Reservation needed</span>}{item.reservation === "confirmed" && <span className="rounded-md bg-[oklch(0.18_0.05_160)] px-2 py-1 text-[10px] text-[var(--positive)]">Reserved</span>}{item.cost && <span className="muted px-1 py-1 text-[10px]">{item.cost}</span>}</div></div>
    <div className="flex items-center"><button aria-label={`Delete ${item.title}`} onClick={() => onDelete(item)} className="grid size-8 place-items-center rounded-lg text-[var(--faint)] opacity-0 transition-opacity hover:bg-[var(--night-soft)] hover:text-white group-hover:opacity-100 focus:opacity-100"><X size={14} /></button><GripVertical className="text-[var(--faint)]" size={15} /></div>
  </motion.article>;
}
...
function DayColumn({ day, items, onDelete }: { day: typeof demoDays[number]; items: DemoItem[]; onDelete: (item: DemoItem) => void }) {
  ...
  return <section ref={droppable.setNodeRef} className={`rounded-2xl border p-3 transition-colors ${droppable.isOver ? "border-[var(--indigo)] bg-[oklch(0.16_0.05_285)]" : "border-[var(--line)] bg-[oklch(0.105_0.01_285)]"}`}>
    <header className="flex items-center justify-between px-1 py-2"><div><div className="flex items-center gap-2"><span className="rounded-md bg-[var(--indigo-soft)] px-2 py-1 text-[10px] font-semibold text-[oklch(0.8_0.1_285)]">DAY {day.ordinal}</span><span className="muted text-xs">{day.date}</span></div><h2 className="mt-2 text-base font-semibold">{day.title}</h2></div><button className="grid size-8 place-items-center rounded-lg text-[var(--muted)] hover:bg-[var(--night-soft)]"><CircleEllipsis size={17} /></button></header>
    <div className="mt-2 space-y-2"><AnimatePresence>{items.map((item) => <ItemCard key={item.id} item={item} onDelete={onDelete} />)}</AnimatePresence><button className="flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-dashed border-[var(--line)] text-xs text-[var(--muted)] hover:border-[var(--faint)] hover:text-white"><Plus size={14} />Add activity or placeholder</button></div>
  </section>;
}
...
function PlannerContent() {
  ...
  return <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={dragEnd}>
    <main className="grid h-[calc(100vh-4rem)] min-h-[680px] xl:grid-cols-[300px_minmax(520px,1fr)_360px]">
      <aside className={`${ideasOpen ? "fixed inset-y-16 left-0 z-40 block w-[300px] shadow-2xl" : "hidden"} overflow-y-auto border-r border-[var(--line)] bg-[var(--night)] p-4 xl:static xl:block xl:w-auto xl:shadow-none`}>
      ...
      <aside className={`${assistantOpen ? "fixed inset-y-16 right-0 z-40 block w-[360px] shadow-2xl" : "hidden"} overflow-y-auto border-l border-[var(--line)] bg-[var(--night)] xl:static xl:block xl:w-auto xl:shadow-none`}>
        <div className="h-[250px] border-b border-[var(--line)]"><JourneyMap compact /></div>
        <div className="p-5"><div className="flex items-center justify-between"><div><p className="eyebrow">Planning signals</p><h2 className="mt-1 text-lg font-semibold">A lighter day</h2></div><button className="xl:hidden" onClick={() => setAssistantOpen(false)}><X size={18} /></button></div>
          <article className="mt-5 rounded-xl border border-[oklch(0.4_0.07_285)] bg-[oklch(0.17_0.04_285)] p-4"><div className="flex gap-3"><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-[var(--indigo-soft)] text-[oklch(0.78_0.12_285)]"><Clock3 size={16} /></span><div><h3 className="text-sm font-semibold">Day 2 has room</h3><p className="muted mt-1 text-xs leading-5">{totalMinutes} minutes planned. Nezu Museum fits well before teamLab.</p><button onClick={() => schedule(ideas[3])} className="mt-3 text-xs font-semibold text-[oklch(0.78_0.12_285)] hover:text-white">Add to Day 2 →</button></div></div></article>
          <article className="mt-3 rounded-xl border border-[var(--line)] p-4"><div className="flex gap-3"><AlertTriangle className="mt-0.5 shrink-0 text-[oklch(0.78_0.15_75)]" size={17} /><div><h3 className="text-sm font-semibold">Reservation to make</h3><p className="muted mt-1 text-xs leading-5">teamLab Borderless is scheduled, but not reserved yet.</p><button className="mt-3 text-xs font-semibold text-[var(--muted)] hover:text-white">Mark reserved</button></div></div></article>
          <article className="mt-3 rounded-xl border border-[var(--line)] p-4"><div className="flex gap-3"><Utensils className="mt-0.5 shrink-0 text-[var(--yarn)]" size={17} /><div><h3 className="text-sm font-semibold">Two open decisions</h3><p className="muted mt-1 text-xs leading-5">Lunch on Day 1 and coffee on Day 2 are still placeholders.</p></div></div></article>
    ...
    <AnimatePresence>{toast && <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} transition={{ duration: reduced ? 0 : .18 }} className="fixed bottom-5 left-1/2 z-[70] flex -translate-x-1/2 items-center gap-4 rounded-xl bg-[var(--ink)] px-4 py-3 text-sm text-[var(--night)] shadow-2xl"><span>Removed {toast.title}</span><button onClick={undo} className="font-bold underline underline-offset-2">Undo</button></motion.div>}</AnimatePresence>
  </DndContext>;
}
