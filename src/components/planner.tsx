...
import { createManualActivity, createPlaceholder, replacePlaceholderWithIdea, saveTripNight, scheduleIdea, updateDayLabel } from "@/app/trips/[tripId]/actions";
...
type AddKind = "activity" | "placeholder" | "stay" | "travel";

const addField = "mt-1 h-9 w-full rounded-lg border border-[var(--line)] bg-white px-2 text-[var(--ink)]";

function AddKindIcon({ kind }: { kind: AddKind }) {
  if (kind === "activity") return <MapPin size={14} />;
  if (kind === "placeholder") return <Sparkles size={14} />;
  if (kind === "stay") return <BedDouble size={14} />;
  return <Bus size={14} />;
}

function ManualActivityForm({ day, tripId }: { day: WorkspaceDay; tripId: string }) {
  return <form action={createManualActivity.bind(null, tripId)} className="grid gap-3 sm:grid-cols-2"><input type="hidden" name="dayId" value={day.id} /><label className="text-xs font-semibold sm:col-span-2">Place or activity<input name="name" required maxLength={160} className={addField} /></label><label className="text-xs font-semibold sm:col-span-2">Details<input name="address" maxLength={240} placeholder="Neighborhood, address, or context" className={addField} /></label><label className="text-xs font-semibold">Priority<select name="priority" defaultValue="would_like" className={addField}><option value="must_do">Must do</option><option value="would_like">Would like</option><option value="if_time">If time</option></select></label><label className="text-xs font-semibold">Time of day<select name="period" defaultValue="afternoon" className={addField}><option value="morning">Morning</option><option value="afternoon">Afternoon</option><option value="evening">Evening</option></select></label><label className="text-xs font-semibold">Exact time <span className="font-normal text-[var(--muted)]">optional</span><input name="exactTime" type="time" className={addField} /></label><label className="text-xs font-semibold">Duration (minutes)<input name="duration" type="number" min="1" max="1440" step="1" defaultValue="60" required className={addField} /></label><label className="text-xs font-semibold sm:col-span-2">Notes<input name="notes" maxLength={1000} placeholder="Tickets, meeting point, or anything to remember" className={addField} /></label><button className="h-9 rounded-lg bg-[var(--indigo)] px-3 text-xs font-semibold text-white sm:col-span-2">Add activity to Day {day.ordinal}</button></form>;
}

function PlaceholderForm({ day, tripId }: { day: WorkspaceDay; tripId: string }) {
  return <form action={createPlaceholder.bind(null, tripId)} className="grid gap-3 sm:grid-cols-2"><input type="hidden" name="dayId" value={day.id} /><label className="text-xs font-semibold">Type<select name="placeholderType" defaultValue="explore" className={addField}><option value="eat">Eat</option><option value="rest">Rest</option><option value="coffee">Coffee</option><option value="explore">Explore</option><option value="buffer">Buffer</option></select></label><label className="text-xs font-semibold">Time of day<select name="period" defaultValue="afternoon" className={addField}><option value="morning">Morning</option><option value="afternoon">Afternoon</option><option value="evening">Evening</option></select></label><label className="text-xs font-semibold">Duration (minutes)<input name="duration" type="number" min="15" max="1440" step="1" defaultValue="60" className={addField} /></label><label className="text-xs font-semibold">Note<input name="notes" placeholder="What still needs deciding?" className={addField} /></label><button className="h-9 rounded-lg bg-[var(--indigo)] px-3 text-xs font-semibold text-white sm:col-span-2">Add placeholder to Day {day.ordinal}</button></form>;
}

function StayForm({ day, nextDay, cities, tripId }: { day: WorkspaceDay; nextDay: WorkspaceDay; cities: WorkspaceCity[]; tripId: string }) {
  const currentCityId = cities.some((city) => city.id === day.cityId) ? day.cityId : cities[0]?.id;
  return <form action={saveTripNight.bind(null, tripId)} className="grid gap-3 sm:grid-cols-2"><input type="hidden" name="dayId" value={day.id} /><input type="hidden" name="nightKind" value="stay" /><label className="text-xs font-semibold sm:col-span-2">Stay in<select name="stayCityId" defaultValue={currentCityId} required className={addField}>{cities.map((city) => <option key={city.id} value={city.id}>{city.name}</option>)}</select></label><label className="text-xs font-semibold sm:col-span-2">Notes <span className="font-normal text-[var(--muted)]">optional</span><input name="notes" maxLength={1000} placeholder="Hotel, neighborhood, or check-in note" className={addField} /></label><p className="text-[10px] leading-4 text-[var(--muted)] sm:col-span-2">This is the night after Day {day.ordinal}, before Day {nextDay.ordinal}. Saving replaces that night’s current stay or travel record.</p><button className="h-9 rounded-lg bg-[var(--indigo)] px-3 text-xs font-semibold text-white sm:col-span-2">Save stay</button></form>;
}

function TravelForm({ day, nextDay, cities, tripId }: { day: WorkspaceDay; nextDay?: WorkspaceDay; cities: WorkspaceCity[]; tripId: string }) {
  const [timing, setTiming] = useState<"daytime" | "overnight">("daytime");
  const reduced = useReducedMotion() ?? false;
  const currentCityId = cities.some((city) => city.id === day.cityId) ? day.cityId : cities[0]?.id;
  const nextCityId = nextDay?.cityId !== currentCityId && cities.some((city) => city.id === nextDay?.cityId) ? nextDay?.cityId : cities.find((city) => city.id !== currentCityId)?.id;
  const routeFields = <><label className="text-xs font-semibold">From<select name="fromCityId" defaultValue={currentCityId} required className={addField}>{cities.map((city) => <option key={city.id} value={city.id}>{city.name}</option>)}</select></label><label className="text-xs font-semibold">To<select name="toCityId" defaultValue={nextCityId} required className={addField}>{cities.map((city) => <option key={city.id} value={city.id}>{city.name}</option>)}</select></label></>;
  return <div><label className="text-xs font-semibold">Travel timing<select aria-label={`Choose travel timing for Day ${day.ordinal}`} value={timing} onChange={(event) => setTiming(event.target.value as "daytime" | "overnight")} className={addField}><option value="daytime">During Day {day.ordinal}</option>{nextDay && <option value="overnight">Overnight into Day {nextDay.ordinal}</option>}</select></label><AnimatePresence mode="wait" initial={false}>{timing === "daytime" ? <motion.form key="daytime" action={createPlaceholder.bind(null, tripId)} initial={reduced ? false : { opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={reduced ? undefined : { opacity: 0, y: -3 }} transition={{ duration: reduced ? 0 : .18 }} className="mt-3 grid gap-3 sm:grid-cols-2"><input type="hidden" name="dayId" value={day.id} /><input type="hidden" name="placeholderType" value="travel" />{routeFields}<label className="text-xs font-semibold">Time of day<select name="period" defaultValue="afternoon" className={addField}><option value="morning">Morning</option><option value="afternoon">Afternoon</option><option value="evening">Evening</option></select></label><label className="text-xs font-semibold">Exact departure <span className="font-normal text-[var(--muted)]">optional</span><input name="exactTime" type="time" className={addField} /></label><label className="text-xs font-semibold">Duration (minutes)<input name="duration" type="number" min="15" max="1440" step="1" defaultValue="120" required className={addField} /></label><label className="text-xs font-semibold">Notes<input name="notes" maxLength={1000} placeholder="Train, flight, drive, or booking detail" className={addField} /></label><button className="h-9 rounded-lg bg-[var(--indigo)] px-3 text-xs font-semibold text-white sm:col-span-2">Add daytime travel to Day {day.ordinal}</button></motion.form> : nextDay ? <motion.form key="overnight" action={saveTripNight.bind(null, tripId)} initial={reduced ? false : { opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={reduced ? undefined : { opacity: 0, y: -3 }} transition={{ duration: reduced ? 0 : .18 }} className="mt-3 grid gap-3 sm:grid-cols-2"><input type="hidden" name="dayId" value={day.id} /><input type="hidden" name="nightKind" value="travel" />{routeFields}<label className="text-xs font-semibold sm:col-span-2">Notes <span className="font-normal text-[var(--muted)]">optional</span><input name="notes" maxLength={1000} placeholder="Night train, flight, or cabin details" className={addField} /></label><p className="text-[10px] leading-4 text-[var(--muted)] sm:col-span-2">This replaces the night after Day {day.ordinal} and counts as overnight travel in Journey Overview.</p><button className="h-9 rounded-lg bg-[var(--indigo)] px-3 text-xs font-semibold text-white sm:col-span-2">Save overnight travel</button></motion.form> : null}</AnimatePresence></div>;
}

function DayAddControl({ day, nextDay, cities, tripId }: { day: WorkspaceDay; nextDay?: WorkspaceDay; cities: WorkspaceCity[]; tripId: string }) {
  const [kind, setKind] = useState<AddKind>("activity");
  const reduced = useReducedMotion() ?? false;
  return <details className="group rounded-xl border border-dashed border-[var(--line)] bg-[var(--surface-soft)] p-3"><summary className="flex cursor-pointer list-none items-center justify-center gap-2 text-xs font-semibold text-[var(--muted)] hover:text-[var(--ink)]"><span className="relative grid size-4 place-items-center"><AnimatePresence mode="popLayout" initial={false}>{<motion.span key={kind} initial={reduced ? false : { scale: .65, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={reduced ? undefined : { scale: .65, opacity: 0 }} transition={{ duration: reduced ? 0 : .16 }} className="absolute"><AddKindIcon kind={kind} /></motion.span>}</AnimatePresence></span>Add to Day {day.ordinal}<ChevronDown size={13} className="transition-transform duration-200 group-open:rotate-180" /></summary><div className="mt-3 border-t border-[var(--line)] pt-3"><label className="text-xs font-semibold">Add<select aria-label={`Choose what to add to Day ${day.ordinal}`} value={kind} onChange={(event) => setKind(event.target.value as AddKind)} className={addField}><option value="activity">Activity</option><option value="placeholder">Placeholder</option>{nextDay && <option value="stay">Stay</option>}{cities.length > 1 && <option value="travel">Travel</option>}</select></label><AnimatePresence mode="wait" initial={false}><motion.div key={kind} initial={reduced ? false : { opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={reduced ? undefined : { opacity: 0, y: -3 }} transition={{ duration: reduced ? 0 : .18 }} className="mt-3">{kind === "activity" ? <ManualActivityForm day={day} tripId={tripId} /> : kind === "placeholder" ? <PlaceholderForm day={day} tripId={tripId} /> : kind === "travel" ? <TravelForm day={day} nextDay={nextDay} cities={cities} tripId={tripId} /> : nextDay ? <StayForm day={day} nextDay={nextDay} cities={cities} tripId={tripId} /> : null}</motion.div></AnimatePresence></div></details>;
}

function DayColumn({ day, nextDay, cities, items, tripId, persisted, registerDay, healthDismissals, onDelete, onDismissHealth, onReplace }: { day: WorkspaceDay; nextDay?: WorkspaceDay; cities: WorkspaceCity[]; items: WorkspaceItem[]; tripId: string; persisted: boolean; registerDay: (dayId: string, node: HTMLElement | null) => void; healthDismissals: ReadonlySet<string>; onDelete: (item: WorkspaceItem) => void; onDismissHealth: (itemId: string) => void; onReplace: (item: WorkspaceItem) => void }) {
  ...
  return <section ref={setNode} data-day-id={day.id} className={`scroll-mt-36 rounded-2xl border p-3 transition-colors ${droppable.isOver ? "border-[var(--indigo)] bg-[var(--indigo-soft)]" : "border-[var(--line)] bg-[var(--surface)]"}`}>
    <header className="flex items-center justify-between px-1 py-2"><div><div className="flex items-center gap-2"><span className="rounded-md bg-[var(--indigo-soft)] px-2 py-1 text-[10px] font-semibold text-[var(--indigo)]">DAY {day.ordinal}</span><span className="muted text-xs">{day.date}</span></div><h2 className="mt-2 text-base font-semibold">{day.title}</h2></div>{persisted ? <details className="relative"><summary aria-label={`More options for Day ${day.ordinal}`} className="grid size-8 cursor-pointer list-none place-items-center rounded-lg text-[var(--muted)] hover:bg-[var(--surface-soft)]"><CircleEllipsis size={17} /></summary><form action={updateDayLabel.bind(null, tripId)} className="absolute right-0 top-10 z-30 w-72 rounded-xl border border-[var(--line)] bg-white p-3 shadow-[0_8px_18px_oklch(0.35_0.04_245/.16)]"><input type="hidden" name="dayId" value={day.id} /><label className="text-xs font-semibold">Day title<input name="label" required minLength={2} maxLength={120} defaultValue={day.title === "Open day" ? "" : day.title} className="mt-1 h-9 w-full rounded-lg border border-[var(--line)] bg-white px-2 text-[var(--ink)]" /></label><button className="mt-3 h-9 w-full rounded-lg bg-[var(--indigo)] px-3 text-xs font-semibold text-white">Save Day {day.ordinal} title</button></form></details> : <button aria-label={`More options for Day ${day.ordinal}`} className="grid size-8 place-items-center rounded-lg text-[var(--muted)] hover:bg-[var(--surface-soft)]"><CircleEllipsis size={17} /></button>}</header>
    <div className="mt-2 space-y-2"><AnimatePresence>{sortItineraryItems(items).map((item) => <ItemCard key={item.id} item={item} healthDismissed={healthDismissals.has(item.id)} onDelete={onDelete} onDismissHealth={onDismissHealth} onReplace={onReplace} />)}</AnimatePresence>{persisted ? <DayAddControl day={day} nextDay={nextDay} cities={cities} tripId={tripId} /> : <button className="flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-dashed border-[var(--line)] text-xs text-[var(--muted)] hover:border-[var(--faint)] hover:text-[var(--ink)]"><Plus size={14} />Add to Day {day.ordinal}</button>}</div>
  </section>;
}
      ...
      <section ref={scrollRef} className="overflow-y-auto bg-[var(--canvas)]">
        ...
        <div className="mx-auto max-w-3xl space-y-7 p-4 md:p-7">{groups.map((group, groupIndex) => {
          ...
          return <section key={`${group.city.id}-${groupIndex}`} aria-labelledby={`city-${group.city.id}-${groupIndex}`}><div className="mb-3 flex items-center justify-between border-b border-[var(--line)] pb-3"><div><p className="eyebrow">Days {group.days[0].ordinal}{group.days.length > 1 ? `–${group.days.at(-1)?.ordinal}` : ""}</p><h2 id={`city-${group.city.id}-${groupIndex}`} className="mt-1 text-lg font-semibold">{group.city.name}</h2><p className="muted mt-1 text-xs"><StayLabel city={group.city} /></p></div><button aria-expanded={!collapsed} onClick={() => setCollapsedCities((current) => { const next = new Set(current); if (next.has(group.city.id)) next.delete(group.city.id); else next.add(group.city.id); return next; })} className="flex items-center gap-1 rounded-lg px-3 py-2 text-xs font-semibold text-[var(--muted)] hover:bg-[var(--surface)] hover:text-[var(--ink)]">{collapsed ? "Expand" : "Collapse"}<ChevronDown className={`transition-transform ${collapsed ? "-rotate-90" : ""}`} size={14} /></button></div>{collapsed ? <button onClick={() => setCollapsedCities((current) => { const next = new Set(current); next.delete(group.city.id); return next; })} className="flex w-full items-center justify-between rounded-xl border border-dashed border-[var(--line)] bg-[var(--surface)] px-4 py-4 text-left"><span><strong className="text-sm">{group.days.length} day{group.days.length === 1 ? "" : "s"} collapsed</strong><span className="muted ml-2 text-xs">{group.city.name}</span></span><ArrowRight size={15} /></button> : <div className="space-y-5">{entries.map((entry) => {
            if (entry.kind === "day") return <DayColumn key={entry.day.id} day={entry.day} nextDay={workspace.days.find((day) => day.ordinal === entry.day.ordinal + 1)} cities={workspace.cities} items={items.filter((item) => item.dayId === entry.day.id)} tripId={workspace.id} persisted={workspace.persistence === "d1"} registerDay={registerDay} healthDismissals={healthDismissals} onDelete={remove} onDismissHealth={(itemId) => setHealthDismissals((current) => new Set(current).add(itemId))} onReplace={replace} />;
            const expanded = expandedRuns.has(entry.id);
            if (expanded) return <div key={entry.id} className="space-y-5">{entry.days.map((day) => <DayColumn key={day.id} day={day} nextDay={workspace.days.find((candidate) => candidate.ordinal === day.ordinal + 1)} cities={workspace.cities} items={[]} tripId={workspace.id} persisted={workspace.persistence === "d1"} registerDay={registerDay} healthDismissals={healthDismissals} onDelete={remove} onDismissHealth={(itemId) => setHealthDismissals((current) => new Set(current).add(itemId))} onReplace={replace} />)}<button onClick={() => setExpandedRuns((current) => { const next = new Set(current); next.delete(entry.id); return next; })} className="mx-auto block rounded-lg px-3 py-2 text-xs font-semibold text-[var(--muted)] hover:bg-[var(--surface)]">Compress open days</button></div>;
