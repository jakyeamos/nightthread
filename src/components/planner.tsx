...
import { createManualActivity, createPlaceholder, replacePlaceholderWithIdea, scheduleIdea, updateDayLabel } from "@/app/trips/[tripId]/actions";
...
function ManualActivityForm({ day, tripId }: { day: WorkspaceDay; tripId: string }) {
  return <details className="rounded-xl border border-dashed border-[var(--line)] bg-[var(--surface-soft)] p-3"><summary className="cursor-pointer text-center text-xs font-semibold text-[var(--muted)]"><Plus className="mr-1 inline" size={14} />Add an activity</summary><form action={createManualActivity.bind(null, tripId)} className="mt-3 grid gap-3 sm:grid-cols-2"><input type="hidden" name="dayId" value={day.id} /><label className="text-xs font-semibold sm:col-span-2">Place or activity<input name="name" required maxLength={160} className="mt-1 h-9 w-full rounded-lg border border-[var(--line)] bg-white px-2 text-[var(--ink)]" /></label><label className="text-xs font-semibold sm:col-span-2">Details<input name="address" maxLength={240} placeholder="Neighborhood, address, or context" className="mt-1 h-9 w-full rounded-lg border border-[var(--line)] bg-white px-2 text-[var(--ink)]" /></label><label className="text-xs font-semibold">Priority<select name="priority" defaultValue="would_like" className="mt-1 h-9 w-full rounded-lg border border-[var(--line)] bg-white px-2 text-[var(--ink)]"><option value="must_do">Must do</option><option value="would_like">Would like</option><option value="if_time">If time</option></select></label><label className="text-xs font-semibold">Time of day<select name="period" defaultValue="afternoon" className="mt-1 h-9 w-full rounded-lg border border-[var(--line)] bg-white px-2 text-[var(--ink)]"><option value="morning">Morning</option><option value="afternoon">Afternoon</option><option value="evening">Evening</option></select></label><label className="text-xs font-semibold">Exact time <span className="font-normal text-[var(--muted)]">optional</span><input name="exactTime" type="time" className="mt-1 h-9 w-full rounded-lg border border-[var(--line)] bg-white px-2 text-[var(--ink)]" /></label><label className="text-xs font-semibold">Duration (minutes)<input name="duration" type="number" min="1" max="1440" step="1" defaultValue="60" required className="mt-1 h-9 w-full rounded-lg border border-[var(--line)] bg-white px-2 text-[var(--ink)]" /></label><label className="text-xs font-semibold sm:col-span-2">Notes<input name="notes" maxLength={1000} placeholder="Tickets, meeting point, or anything to remember" className="mt-1 h-9 w-full rounded-lg border border-[var(--line)] bg-white px-2 text-[var(--ink)]" /></label><button className="h-9 rounded-lg bg-[var(--indigo)] px-3 text-xs font-semibold text-white sm:col-span-2">Add activity to Day {day.ordinal}</button></form></details>;
}
